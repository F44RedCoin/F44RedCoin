...
def main():
    ...
    tmpdir = "%s/f44redcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and f44redcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "f44redcoind"]) is not None:
    ...
    #Set env vars
    if "F44REDCOIND" not in os.environ:
        os.environ["F44REDCOIND"] = build_dir + '/src/f44redcoind' + exeext
        os.environ["F44REDCOINCLI"] = build_dir + '/src/f44redcoin-cli' + exeext
