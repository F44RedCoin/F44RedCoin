...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  F44R), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in F44R per 1,000
...
- #16521 Use the default maxfeerate value as F44R/kB (Remagpie)
...
- #16929 follow-up to rpc: default maxfeerate value as F44R/kB (jonatack)
