F44RedCoin Core version 0.13.2 is now available from:
  ...
  <https://download.f44redcoin.org/f44redcoin-0.13.2.1/>
...
F44RedCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
F44RedCoin Core 0.13.2 also introduces new default policy limits on the length and
...
Then, to tunnel a SSL connection on 211587 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 211587 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /f44redcoinrpc>
        ProxyPass http://127.0.0.1:11587/
        ProxyPassReverse http://127.0.0.1:11587/
...
- `f44redcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `f44redcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ...
  currently being researched by Bitcoin and F44RedCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the F44RedCoin
client to F44RedCoin Core 0.13.2 to signal for segwit support. When a
