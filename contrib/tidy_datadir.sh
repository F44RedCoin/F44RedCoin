...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete F44RedCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no F44RedCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old F44RedCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected F44RedCoin 0.7 datadir."
    ;;
  3)
    echo "Detected F44RedCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected F44RedCoin 0.8 datadir."
    ;;
esac
